...
public class BOJ17836 {
    ...
    static class Man{
        ...
        public Man(int x, int y, int t, boolean s) {
            this.x = x;
            this.y = y;
            this.t = t;
            this.s = s;
    ...
    static void compute(){
        ...
        q.add(new Man(0, 0, 0, false));
        visited[0][0][0] = true;
        while (!q.isEmpty()) {
            Man man = q.poll();
            if (man.x == N - 1 && man.y == M - 1) {
                min = Math.min(min, man.t);
            }
            for (int i = 0; i < 4; i++) {
                int nx = man.x + dx[i];
                int ny = man.y + dy[i];
                if (isAvailable(nx, ny)) {
                    if (!man.s) {
                        if (!visited[nx][ny][0] && map[nx][ny] == 0) {
                            ...
                            q.add(new Man(nx, ny, man.t + 1, false));
                        } else if (!visited[nx][ny][0] && map[nx][ny] == 2) {
                            visited[nx][ny][0] = true;
                            q.add(new Man(nx, ny, man.t + 1, true));
                        }
                    }else{
                        if (!visited[nx][ny][1]) {
